...
class JdbookSpider(scrapy.Spider):
    ...
    def parse(self, response):
        dt_list = response.xpath('//div[@class="mc"]/dl/dt') # 图书大分类列表
        # 图书大分类里面取出图书每一个大分类
        ...
        for dt in dt_list:
            item = dict()
            item['b_cate'] = response.xpath('./a/text()').extract_first() # 大分类名称
            # ./表示选定当前标签，following-sibling::dd[1]表示向后取兄弟标签，取第一个dd标签
            em_list = dt.xpath('./following-sibling::dd[1]/em') # 大分类下的小分类列表
            for em in em_list:
