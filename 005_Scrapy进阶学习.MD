...
- 最常用的是下载器中间件：
    ...
    - 不同的方法的返回值不同，参考下面代码
    - process_request(self, request, spider)是一个默认方法，当每一个request通过该中间件时候都会启动该方法
    - process_response(self, request, response, spider)是一个默认方法，当每一个response通过该中间件时候都会启动该方法
    - 两种方法必须有返回值：
```python 
